...
register_tools() ->
    ...
    ok = erlmcp_stdio:add_tool(<<"query">>,
        <<"Prove a Prolog goal against a cached codebase and return all "
          ...
          "defines), calls(Caller, CallerArity, CallSpec, File, Line) where CallSpec "
          ...
          "truly_uncalled/3 (+all_truly_uncalled/1 - excludes "
          "entry_point/3, so exported API and OTP callbacks are not dead "
          "code; edit runtime_entry_point/2 for an entry point with no "
          "-export behind it, like an -on_load hook), "
          "entry_point/3 (export/4 facts + runtime_entry_point/2), "
          "banned_call/4 "
